...
- **nav-contract — fixed, no longer a WARN (`ferraroroberto/github-copilot-usage#22`).** The bottom-tab pill itself still doesn't apply — genuine single-view dashboard, only `<nav>` is the period segmented control (`static/index.html`, `class="seg"`), not primary navigation, no `_vendored/nav/` to adopt. But the shell mechanics the nav contract also requires (viewport-anchored `100dvh`/`100lvh` sizing, `env(safe-area-inset-bottom)`, `body:has(dialog[open])` hide-under-dialog, standalone fixed-inset `.app` scroller) are worth carrying regardless of the tab bar for an installed PWA, so `static/styles.css` now has a `@media (display-mode: standalone)` block making `.app` the fixed-inset scroller. If this app ever grows a second top-level view, still adopt `_vendored/nav/` verbatim on top of this shell — never re-author it.
- **Re-audit log** — append a line whenever a re-audit lands (don't rewrite the entry), so the next auditor sees they were checked, not missed. `static/` was byte-unchanged through 2026-09-03; #22 (2026-09-10) is the first change:
  ...
  - 2026-09-10 — `ferraroroberto/github-copilot-usage#22`; nav-contract WARN resolved by adding the standalone fixed-inset `.app` shell (see above) — `design_lint contracts` now reports PASS. `hit-target` WARN remains the sole accepted exception.
